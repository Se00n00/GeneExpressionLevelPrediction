...
class Convo_attention_layer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, pool_kernel_size, embed_dim, model_dim, stride=1, pool_stride=None):
        ...
        self.maxpool = nn.MaxPool1d(pool_kernel_size, stride=pool_stride)

    def forward(self, input):
        # input: [B, 4, 100]
        conv_out = self.relu(self.conv(input))                 # [B, out_channels, 100]
        trans_out = self.relu(self.transposed_conv(conv_out))  # [B, in_channels, 100] = [B, 4, 100]
        ...
        pooled = self.maxpool(output)           # If pool_kernel_size=1, shape remains [B, 4, 100]
